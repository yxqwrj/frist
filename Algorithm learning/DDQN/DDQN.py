...
class DDQN:
    def __init__(self, alpha, state_dim, action_dim, fc1_dim, fc2_dim, ckpt_dir,
                 gamma=0.99, tau=0.005, epsilon=1.0, eps_end=0.01, eps_dec=5e-6,
    ...
    def remember(self, state, action, rewards, state_, done):
        self.memory.store_transition(state, action, rewards, state_, done)

    # 修改后的 choose_action 方法
    def choose_action(self, state, isTrain=True):
        state = T.tensor([state], dtype=T.float).to(device)
        actions = self.q_eval.forward(state)
        action = T.argmax(actions).item()
        if (np.random.random() < self.epsilon) and isTrain:  #不会~_`~
            action = np.random.choice(self.action_space)

        return action

    def decrement_epsilon(self):
        self.epsilon = max(self.epsilon - self.eps_dec, self.eps_min)  # 逐渐递减 epsilon 直到最小值
